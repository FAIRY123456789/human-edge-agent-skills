# one-week-founder-mode

> **One main bet, two microbets, a kill list**

Compress an overloaded startup/research/job-search week into one outcome that changes the next decision, with evidence due by week end.

**Category:** AI Native Bets
**Keywords:** founder, weekly-planning, execution, startup, focus

## Use it when

The task described in [`SKILL.md`](SKILL.md) keeps recurring and a generic assistant response would miss the workflow, guardrails, or output contract encoded here.

## Preview / install

```bash
gh skill preview FAIRY123456789/human-edge-agent-skills one-week-founder-mode
gh skill install FAIRY123456789/human-edge-agent-skills one-week-founder-mode

# Or via the Skills CLI ecosystem
npx skills add https://github.com/FAIRY123456789/human-edge-agent-skills --skill one-week-founder-mode
```

Inspect `SKILL.md` and any `references/`, `assets/`, or `scripts/` before installing. Agent Skills can contain executable resources.

## Proof status

This repository does not claim external adoption yet. The next evidence target is real task completion by external users, followed by repeat use, useful issues/PRs, or measurable workflow improvement.
