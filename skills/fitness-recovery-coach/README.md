# fitness-recovery-coach

> **Training plans that respect recovery instead of forcing the calendar**

Plan strength, hypertrophy, body-composition, recovery, and habit adjustments while preserving medical-safety boundaries.

**Category:** Young Life
**Keywords:** fitness, recovery, training, habits

## Use it when

The task described in [`SKILL.md`](SKILL.md) keeps recurring and a generic assistant response would miss the workflow, guardrails, or output contract encoded here.

## Preview / install

```bash
gh skill preview FAIRY123456789/human-edge-agent-skills fitness-recovery-coach
gh skill install FAIRY123456789/human-edge-agent-skills fitness-recovery-coach

# Or via the Skills CLI ecosystem
npx skills add https://github.com/FAIRY123456789/human-edge-agent-skills --skill fitness-recovery-coach
```

Inspect `SKILL.md` and any `references/`, `assets/`, or `scripts/` before installing. Agent Skills can contain executable resources.

## Proof status

This repository does not claim external adoption yet. The next evidence target is real task completion by external users, followed by repeat use, useful issues/PRs, or measurable workflow improvement.
