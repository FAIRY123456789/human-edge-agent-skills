...
Codex packaging and runtime validation are recorded in the [Codex runtime report](docs/CODEX_RUNTIME_VALIDATION.md). Cross-platform state is summarized in [Cross-Runtime Marketplace Status](docs/CROSS_RUNTIME_MARKETPLACE_STATUS.md). The [Plugin Submission README](docs/PLUGIN_SUBMISSION_README.md) provides one copy-ready source for Claude and Cursor form fields; platform-specific records remain available for [OpenAI](docs/OPENAI_PLUGIN_SUBMISSION_FINAL.md), [Cursor](docs/CURSOR_MARKETPLACE_SUBMISSION_FINAL.md), [Claude](docs/CLAUDE_COMMUNITY_SUBMISSION_FINAL.md), [GitHub Awesome Copilot](docs/GITHUB_AWESOME_COPILOT_SUBMISSION_FINAL.md), [skills.sh](docs/SKILLS_SH_STATUS.md), and the [Gemini CLI feasibility decision](docs/GEMINI_CLI_GALLERY_FEASIBILITY.md). Cursor 3.19.13 discovered all four local plugins and five packaged Skills; its five behavior prompts still require a user visual check. Claude Code validation and runtime remain untested because only Claude Desktop, not the Claude Code CLI, is available on the validation host. No Claude or Cursor Marketplace submission has been made.
